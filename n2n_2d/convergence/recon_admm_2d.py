...
import argparse
parser = argparse.ArgumentParser(description = 'noise2noise unsupervised reconstruction, admm')
...
parser.add_argument('--optimizer', dest='optimizer', type=str, default='lbfgs')
...
parser.add_argument('--nSubIters', dest='nSubIters', type=int, default=10)
...
parser.add_argument('--gamma', dest='gamma', type=float, default = 4e-4)
parser.add_argument('--betaRecon', dest='betaRecon', type=float, default = 20, 
                    help='gamma * betaRecon is the rho in admm algorithm')
...
if sys.argv[0] != 'recon_admm_2d.py':
...
#                               '--vmax', '1.15',
                              ...
                              '--outDir', '../../../train/recon/n2n_2d/convergence/admm_patch_pre_lbfgs/'])
else:
    showPlots = False
...
with tf.control_dependencies(update_ops):
    if args.optimizer == 'adam':
        ...
        train_step = tf.contrib.opt.ScipyOptimizerInterface(net.loss, method='L-BFGS-B', options={'maxiter': args.nSubIters})
    else:
        raise ValueError('args.optimizer must be either adam or lbfgs')
...
normImg = reconNet.cSiddonFanNormImg3d(prjs, weights) / projectorNorm / projectorNorm
...
def SQSOneStep(reconNet, x, x_nesterov, z, masks, prj, weight, normImg, projectorNorm, args, verbose = 0):
    ...
    for i in range(args.nSubsets):
        ...
        fp = reconNet.cSiddonFanProjection3d(x_new, curAngles) / projectorNorm
        dprj = fp - prj[:, :, i*nAnglesPerSubset:(i+1)*nAnglesPerSubset, :] / projectorNorm
        bp = reconNet.cSiddonFanBackprojection3d(curWeight * dprj, curAngles) / projectorNorm
    ...
    # get loss function
    fp = reconNet.cSiddonFanProjection3d(x, angles) / projectorNorm
...
for iIter in range(args.nIters):
    # SQS
    x, x_nesterov, lossData = SQSOneStep(reconNet, x, x_nesterov, z - u, masks, prjs, weights, normImg, projectorNorm, args, 
                                         verbose = showPlots)
    
    # network
    if args.optimizer == 'adam':
        for iSubIter in range(args.nSubIters):
            patches = ExtractPatches([imgs1, imgs2, x + u, masks], net.imgshape, args.nPatchesPerImg)
...
if showPlots:
    vals = np.array(vals)
    plt.loglog(0.5 * (vals[:, 0] + args.gamma * (vals[:,2])))
